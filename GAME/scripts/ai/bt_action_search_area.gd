...
var _last_replan_time: float = 0.0
...
## Nav target replanning interval (seconds) — avoids per-tick recalculation.
const REPLAN_INTERVAL: float = 0.4
...
## Lower confidence = longer scan pause (more uncertain, look harder).
func _phase_secure(delta: float, npc: NPC, anchor: Vector2) -> Status:
	if blackboard.get_var(&"is_interacting", false):
		if npc.movement_component: npc.movement_component.move_velocity(Vector2.ZERO)
		return RUNNING

	# Confidence-adjusted scan duration: low confidence = scan longer
	var confidence: float = IntelConfidence.get_current_confidence(blackboard)
	var adjusted_scan: float = SCAN_DURATION + (1.0 - confidence) * 1.5
...
## Lower confidence = wider search radius (less certain where player is).
func _phase_sweep(delta: float, npc: NPC, anchor: Vector2) -> Status:
	...
	# ── Confidence-scaled search radius ───────────────────────────────
	var confidence: float = IntelConfidence.get_current_confidence(blackboard)
	# Low confidence = wider search (1.0x–2.0x multiplier)
	var uncertainty_mult: float = 1.0 + (1.0 - confidence)

	# Determine search radius based on phase
	var is_phase_b: bool = _time_searching <= PHASE_B_END
	var current_radius: float
	if is_phase_b:
		current_radius = search_radius * uncertainty_mult
	else:
		...
		current_radius *= uncertainty_mult
	...
	# ── Throttled replanning ──────────────────────────────────────────
	var now: float = Time.get_ticks_msec() / 1000.0
	var should_replan: bool = not _target_set
	if _target_set and (now - _last_replan_time) >= REPLAN_INTERVAL:
		# Check if we've arrived — only then replan
